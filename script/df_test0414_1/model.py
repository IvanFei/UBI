...
# model 
from sklearn import linear_model, svm, ensemble, neighbors, neural_network
...
#    ]
    ...
    # data splite
    split = model_selection.ShuffleSplit(n_splits=3, random_state=0)
    
    grid_n_estimator = [100, 300, 600, 1000, 1300]
    grid_ratio = [.1, .25, .5, .75, 1.0]
    grid_learn = [.01, .04, .08, .25]
    grid_max_depth = [2, 4, 8, None]
    ...
    vote_est = [
        ...
        #MLPRegressor
        ('mlp', neural_network.MLPRegressor(hidden_layer_sizes=(50, 100, 20,), max_iter=2000)),
    ...
    for rlf, param in zip(vote_est, grid_param):
        ...
        best_search = model_selection.GridSearchCV(estimator=rlf[1], 
                param_grid=param, cv=split, scoring='neg_mean_squared_error', n_jobs=n_jobs)
        best_search.fit(X, Y)
